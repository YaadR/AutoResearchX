...
2. **Separated concerns** — Problem-specific logic lives in the `train/` directory, while methodology and constraints are isolated in guidance files.
...
- **`runs/trials/`** — Snapshot of every run's code, outputs, and metadata. Enables reproducibility and historical comparison.

- **`runs/best/`** — The currently kept best model configuration. Contains the best code, outputs, and metrics. Maintained by the agent; used as reference for next iterations.
...
3. **Innovate** — Modify files in the `train/` directory (including utilities or debug scripts) and implement the hypothesis.
...
- **Domain-specific innovation** lives entirely in the `train/` directory—including debug scripts, utilities, and any experimental files.
...
3. **Replace the problem-specific components:**
   ...
   - Write initial baseline files in the `train/` directory that implement a baseline for your domain.
...
4. Run your first experiment and observe the loop in action.p in action. **Disciplined Experimentation:** Forces coherent hypotheses, not random parameter sweeps.
- **Audit Trail:** Complete history enables debugging, reproducibility, and learning from past decisions.
- **Adaptive Autonomy:** Supports both structured (junior) and creative (senior) modes.
- **Extensible:** New files, utilities, and domain-specific tools integrate naturally.
- **Sustainable:** Scales to hundreds of runs without losing clarity or context.
...
2. Follow the quick-start commands to set up the environment.
3. Review `V0/program.md` for complete methodology.
4. Run your first experiment and observe the loop in action.p in action.